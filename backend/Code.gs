...
function getHeadersForSheet(sheetName) {
  switch (sheetName) {
    ...
    case '회원_마스터': return ['이름', '시작일', '장애비장애구분', '구분', '상태', '팀명', '사업명', '메모', '요일', '그룹구분'];
...
function addMember(data) {
  ...
  const groupDiv = data.그룹구분 || data['그룹/반 구분'] || data.그룹반구분 || '';
  sheet.appendRow([
    data.이름, data.시작일, data.장애비장애구분 || '비장애', data.구분 || '개별', data.상태 || '활성', data.팀명 || '', data.사업명 || '', data.메모 || '', daysVal, groupDiv
  ]);
  invalidateCache();
  return true;
}
...
function importMembersCSV(csvData) {
  ...
  const newRows = csvData.map(row => [
    ...
    row.그룹구분 || row['그룹/반 구분'] || row.그룹반구분 || ''
  ]);
  if (newRows.length > 0) {
